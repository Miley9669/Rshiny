...
### Copied from the solutions website ###
...
server <- function(input, output, session) {
  ...
  ### *** Don't understand the code below. Not sure how the %% is functioning. How is that making it so that 
  #we don't go outside the interval? Also, not sure why we use "-2" when going back. Why not just use -1 and remove the +1?
  #^^^The modulo operator is used to ensure that the selected index is always w/in the frame. It works such that you go back
  #to the first index once you hit the limit. E.g., limit = 50. Formula uses n %% limit, so if you're less than 50, then 
  #n %% limit = n, but once you get to n = 51, 51 %% 50 = 1, so it resets
  
  #^^^The "-2" piece is needed to deal with the case where you're at the edge of the possible values. If you're at n = 1, then 
  #you want to end up at n=50 when you go back a step. This allows you to get that value. 
